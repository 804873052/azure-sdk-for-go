...
// PolicyPricingsListForManagementGroupPager provides operations for iterating over paged responses.
type PolicyPricingsListForManagementGroupPager struct {
	client    *PolicyPricingsClient
	current   PolicyPricingsListForManagementGroupResponse
	err       error
	requester func(context.Context) (*policy.Request, error)
	advancer  func(context.Context, PolicyPricingsListForManagementGroupResponse) (*policy.Request, error)
}

// Err returns the last error encountered while paging.
func (p *PolicyPricingsListForManagementGroupPager) Err() error {
	return p.err
}

// NextPage returns true if the pager advanced to the next page.
// Returns false if there are no more pages or an error occurred.
func (p *PolicyPricingsListForManagementGroupPager) NextPage(ctx context.Context) bool {
	var req *policy.Request
	var err error
	if !reflect.ValueOf(p.current).IsZero() {
		if p.current.PolicyPricingListResult.NextLink == nil || len(*p.current.PolicyPricingListResult.NextLink) == 0 {
			return false
		}
		req, err = p.advancer(ctx, p.current)
	} else {
...
// PageResponse returns the current PolicyPricingsListForManagementGroupResponse page.
func (p *PolicyPricingsListForManagementGroupPager) PageResponse() PolicyPricingsListForManagementGroupResponse {
	return p.current
}

// PolicyPricingsListPager provides operations for iterating over paged responses.
type PolicyPricingsListPager struct {
	client    *PolicyPricingsClient
	current   PolicyPricingsListResponse
	err       error
	requester func(context.Context) (*policy.Request, error)
	advancer  func(context.Context, PolicyPricingsListResponse) (*policy.Request, error)
}

// Err returns the last error encountered while paging.
func (p *PolicyPricingsListPager) Err() error {
	return p.err
}

// NextPage returns true if the pager advanced to the next page.
// Returns false if there are no more pages or an error occurred.
func (p *PolicyPricingsListPager) NextPage(ctx context.Context) bool {
	var req *policy.Request
	var err error
	if !reflect.ValueOf(p.current).IsZero() {
		if p.current.PolicyPricingListResult.NextLink == nil || len(*p.current.PolicyPricingListResult.NextLink) == 0 {
...
// PageResponse returns the current PolicyPricingsListResponse page.
func (p *PolicyPricingsListPager) PageResponse() PolicyPricingsListResponse {
	return p.current
}
